...
namespace System.IO.Abstractions.TestingHelpers.Tests
{
    public class MockFileArgumentPathTests
    {
        private static IEnumerable<object[]> GetFileSystemActionsForArgumentNullException()
        {
            ...
            yield return
                new object[] {(Action<FileBase>) (fs => fs.AppendAllLines(null, new[] {"does not matter"}, Encoding.ASCII))};
            ...
            yield return
                new object[] {(Action<FileBase>) (fs => fs.WriteAllLines(null, new[] {"does not matter"}, Encoding.ASCII))};
            yield return new object[] {(Action<FileBase>) (fs => fs.WriteAllLines(null, new[] {"does not matter"}.ToArray()))};
            yield return
                new object[]
                    {(Action<FileBase>) (fs => fs.WriteAllLines(null, new[] {"does not matter"}.ToArray(), Encoding.ASCII))};
            ...
            yield return
                new object[]
                    {(Action<FileBase>) (fs => fs.Open(null, FileMode.OpenOrCreate, FileAccess.Read, FileShare.Inheritable))};
