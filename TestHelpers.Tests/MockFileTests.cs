...
namespace System.IO.Abstractions.TestingHelpers.Tests
{
    ...
    public class MockFileTests
    {
        ...
        [Test]
        public void MockFile_MoveTo_NonExistentDestination_SourceFileShouldNotExistDestinationShouldExist()
        {
            var fileSystem = new MockFileSystem();
        ...
        [Test]
        public void MockFile_MoveTo_NonExistentDestinationFolder_ShouldThrowDirectoryNotFoundException()
        {
            var fileSystem = new MockFileSystem();
            var sourcePath = XFS.Path(@"c:\temp\file.txt");
            ...
            Assert.Throws<DirectoryNotFoundException>(() => fileSystem.File.Move(sourcePath, destinationPath));
        }

        [Test]
        public void MockFile_MoveTo_ExistingDestination_ShouldThrowExceptionAboutFileAlreadyExisting()
        {
            var fileSystem = new MockFileSystem();
            var sourcePath = XFS.Path(@"c:\temp\file.txt");
            var destinationPath = XFS.Path(@"c:\temp2\file.txt");
            ...
            Assert.Throws<IOException>(() => fileSystem.File.Move(sourcePath, destinationPath));
        }

        [Test]
        public void MockFile_MoveTo_SameSourceAndTargetIsANoOp()
        {
            var fileSystem = new MockFileSystem();
            var path = XFS.Path(@"c:\temp\file.txt");
        ...
        [Test]
        public void MockFile_MoveTo_SameSourceAndTargetThrowsExceptionIfSourceDoesntExist()
        {
            var fileSystem = new MockFileSystem();
            var path = XFS.Path(@"c:\temp\file.txt");

            Assert.Throws<FileNotFoundException>(() => fileSystem.File.Move(path, path));
        }

        [Test]
        public void MockFile_MoveTo_ThrowsExceptionIfSourceDoesntExist()
        {
            var fileSystem = new MockFileSystem();
            var sourcePath = XFS.Path(@"c:\temp\file.txt");
            var destinationPath = XFS.Path(@"c:\temp\file2.txt");

            Assert.Throws<FileNotFoundException>(() => fileSystem.File.Move(sourcePath, destinationPath));
        }

        [Test]
        public void MockFile_CopyTo_ThrowsExceptionIfSourceDoesntExist()
        {
            var fileSystem = new MockFileSystem();
            var sourcePath = XFS.Path(@"c:\temp\file.txt");
            var destinationPath = XFS.Path(@"c:\temp\file2.txt");

            Assert.Throws<FileNotFoundException>(() => fileSystem.File.Copy(sourcePath, destinationPath));
        }
